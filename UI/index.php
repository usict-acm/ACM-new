              ...
              if (data[i].fieldType == "shortAns") {
                ...
                if (data[i].required == 1) {
                  divTag.innerHTML += `<div class="wrap-input100  validate-input">
                                        <input id="field${i}" class="input100 input-group" type="text" style="height: 62px; border-radius: 25px;" required placeholder="${data[i].fieldName} *" /> 
                                        </div>`;
                } else {
                  divTag.innerHTML += `<div class="wrap-input100  validate-input">
                                      <input id="field${i}" class="input100 input-group" type="text" style="height: 62px; border-radius: 25px;" required placeholder="${data[i].fieldName}" /> 
                ...
                if (data[i].required == 1) {
                  divTag.innerHTML += `<div class="wrap-input100  validate-input">
                                      <textarea id="field${i}" class="input100 input-group" rows="4" cols="50" type="text" style="height: 62px;" required placeholder="${data[i].fieldName} *" ></textarea> 
                                      </div>`;    
                } else {
                  divTag.innerHTML += `<div class="wrap-input100  validate-input">
                                        <textarea id="field${i}" class="input100 input-group" rows="4" cols="50" type="text" style="height: 62px;" required placeholder="${data[i].fieldName}" ></textarea> 
                ...
                if (data[i].required == 1) {
                  divTag.innerHTML += `<div id="field${i}"><label>${data[i].fieldName} *</label>`;                
                } else {
                  divTag.innerHTML += `<div id="field${i}"><label>${data[i].fieldName}</label>`;                
                }
                for (let j = 0; j < nameArr.length; j++) {
                  divTag.innerHTML += `<input id="field${i}${j}" type="checkbox" value="${nameArr[j]}">
                                        <label> ${nameArr[j]} </label><br>`;
                }
      ...
      for(let i=0; i<data.length; i++){
        ...
        if(data[i].fieldType == "checkbox"){
          ...
          if(count == arr.length && data[i].required == 1){
            ...
            setTimeout(function(){$(".fieldCheckCheckbox").css('display','none'); }, 4000);
            temp=0;
          } else {
            let text = temp1.join(",");
          ...
          if($(`#${field}`).val() == "" && data[i].required == 1){
            $(`#${fieldRequired}`).css('display','block');
            setTimeout(function(){$(".fieldCheck").css('display','none'); }, 4000);
            temp = 0;
          }    
