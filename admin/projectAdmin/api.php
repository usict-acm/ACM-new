...
function postproject()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        if (in_array($filecheck, $fileextstored)) {
            $destinationfile = 'upload/projects/' . $filename;
            $uploadLocation = '../../upload/projects/' . $filename;
            move_uploaded_file($filetemppath, $uploadLocation);

            $sql = "INSERT INTO `projects` (`projectTitle`, `coverImage`, `content`) VALUES ('$title', 'upload/projects/$filename' , '$content', current_timestamp(), current_timestamp(), '$isDraft' , '$tags' , '$approved');";
...
/*function approveRequest()
{
    ...
    $db = $database->connect();
    $id = $_GET["Id"];
    $sql = "UPDATE `projects` SET approved = '1' WHERE projectId='" . $id . "'";    
    ...
    if ($db->query($sql) == true) {
        projectApprovedMail($projectData["projectTitle"], $projectData["projectId"]);
        echo json_encode(
...
function rejectRequest()
{
    ...
    $db = $database->connect();
    $id = $_GET["Id"];
    $sql = "UPDATE `projects` SET approved = '0',isDraft='1' WHERE projectId='" . $id . "'";
    ...
    if ($db->query($sql) == true) {
        projectRejectedMail($projectData["projectTitle"], $projectData["projectId"]);
        echo json_encode(
            array('message' => 'Form has been submitted')
        );
    } else {
