...
export default function Blog() {
  ...
  useEffect(() => {
    ...
    setFilterData(() => {
      return blogs
        .filter((draft) => draft.isDraft === data.disabled)
        .filter((val) => {
          if (searchItem === "") {
            return val;
          } else if (
            val.blogTitle.toLowerCase().includes(searchItem.toLowerCase())
          ) {
            return val;
          }
        });
                  ...
                  <Col xs="4" className="p-0">
                    ...
                    >
                      <DropdownToggle className="drop-text" caret>
                        {data.select} (
                        {data.disabled ? entries.draft : entries.published})
                      </DropdownToggle>
                      <DropdownMenu>
                        <DropdownItem
                          onClick={() =>
                            setData({ select: "Published", disabled: false })
                          }
                        >
                          Published
                        </DropdownItem>
                        ...
                        <DropdownItem
                          onClick={() =>
                            setData({ select: "Draft", disabled: true })
                          }
                        >
                          Draft
                        </DropdownItem>
